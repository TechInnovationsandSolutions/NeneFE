...
const Navbar = ({setOpenBar, setOpenSearch,openSearch}) => {
  ...
  return (
    <nav className='fixed flex flex-col lg:z-40 lg:h-[100px] lg:border-b-[0.5px] border-[#D7D7D7] w-full bg-white'>
    <div className='flex w-full'>
      ...
      <div className='lg:flex items-center justify-between w-full px-9 pt-9 pb-4  hidden'>
          <div className='h-10 xl:w-[400px]  flex px-5 py-2 bg-[#F2F2F2] rounded-sm '>
            <input className='w-full bg-inherit h-full outline-none' value={search} onChange={e => setSearch(e.target.value)}  placeholder='search . . .'/>
            <Image src={searchicon} alt='search' onClick={submitSearch(search)} />     
          </div>
      ...
      <div className='flex lg:hidden ml-auto p-5 gap-10 items-center'>
        ...
        <Image src={searchicon} alt='search' onClick={() => setOpenSearch(true)} className={`${openSearch && 'hidden'}`} />
      </div>
    </div>
    <div className=''>
      <SearchBarMobile openSearch={openSearch} setOpenSearch={setOpenSearch} />
    </div>
  </nav>
  )
}
