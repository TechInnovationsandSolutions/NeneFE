...
const OrderFAQ = () => {
  ...
  useEffect(() => {
    const initialItems = faqList.slice(0, itemsPerPage);
  ...
  const handleLoadMoreBtnClick = () => {
    ...
    const newItems = faqList.slice(startIndex, endIndex);
    ...
    <div className="flex flex-col w-full h-auto">
        ...
        {items.map((item) => {
          return (
            ...
            >
              ...
              <Image src={carretRightIcon} alt="" className="w-auto h-auto" width={24} height={24} />
            </Link>
          );
        })}
      </section>
      ...
      <section className="flex flex-col w-full h-auto ">
        <div className="bg-white flex flex-col w-full items-center gap-6 justify-center px-6 py-10">
          <p className="text-[14px] lg:text-[18px] leading-[23px]">
            {items.length} of {faqList.length}
          </p>
          <button
